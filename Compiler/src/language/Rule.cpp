...
// EDIT NEW RULES FOR WEAK

/*aspc::Rule::Rule(const std::vector<aspc::Atom> &head, const std::vector<aspc::Literal> &body, const std::string weight, const std::string level, const std::set<std::string> termsInWhichPay)
{
}*/

aspc::Rule::Rule(const std::vector<aspc::Atom> &head, const std::vector<aspc::Literal> &body, const std::vector<aspc::ArithmeticRelation> &arithmeticRelations, const std::vector<aspc::ArithmeticRelationWithAggregate> &arithmeticRelationsWithAggregate, const std::string weight, const std::string level, const std::set<std::string> termsInWhichPay)
{
}

bool aspc::Rule::equalBodyExpect(const aspc::Rule &other, const std::unordered_set<std::string> &forbidden)
{
    if (bodyLiterals.size() != other.bodyLiterals.size())
        return false;
    if (arithmeticRelations.size() != other.arithmeticRelations.size())
        return false;
    // TODO: Add Aggregates
    ...
    {
        if (forbidden.count(lit.getPredicateName()))
            continue;
        bool found = false;
...
aspc::Rule::Rule(const Rule &other) : head(other.head), bodyLiterals(other.bodyLiterals), ruleId(other.ruleId), arithmeticRelations(other.arithmeticRelations), arithmeticRelationsWithAggregate(other.arithmeticRelationsWithAggregate), orderedBodyByStarters(other.orderedBodyByStarters), orderedBodyIndexesByStarters(other.orderedBodyIndexesByStarters)
{
    for (unsigned i = 0; i < bodyLiterals.size(); i++)
    {
        formulas.push_back(new Literal(bodyLiterals.at(i)));
...
void aspc::Rule::print() const
{
    ...
    for (const ArithmeticRelationWithAggregate &arithmeticRelationsWithAggregate : arithmeticRelationsWithAggregate)
    {
        arithmeticRelationsWithAggregate.print();
        cout << " ";
    }
    cout << "\n";*/
}
...
void aspc::Rule::orderBodyFormulasFromStarter(unsigned starter, std::vector<unsigned> &orderedBodyFormulas) const
{
    ...
    while (orderedBodyFormulas.size() < size)
    {
        ...
        if (selectedIndex == -1)
        {
            std::cout << "Error:\tUnable to order rule ";
            print();
            exit(180);
        }
...
bool aspc::Rule::extractLabeledFormula(const std::vector<int> &formulaLabeling, std::unordered_map<std::string, int> &predToComponent, const std::vector<int> &sccLabel, int label, std::vector<bool> &extraction, std::unordered_set<std::string> &positiveEDBVar) const
{
    ...
    std::cout << "Found " << countExtractedLiteral << " for rule ";
    print();
