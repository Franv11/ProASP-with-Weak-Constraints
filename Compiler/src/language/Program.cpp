...
// do to add rewrittenRule
aspc::Program::Program() : rules({}), facts({}), reWrittenWeaks({}), predicates({}), aggregatePredicates({}), rulesForPredicate({}), rules_by_type({}), stratified(true), tight(true)
{
}
aspc::Program::Program(const aspc::Program &p) : facts(p.facts), predicates(p.predicates), aggregatePredicates(p.aggregatePredicates), stratified(p.stratified), tight(p.tight)
{
    ...
    for (unsigned i = 0; i < p.getReWrittenWeakSize(); i++)
    {
        addRewrittenWeak(p.getRewrittenWeak(i));
    }
}
...
// new method addWeak
void aspc::Program::addWeakConstraint(const aspc::Rule &w)
{
    weakConstraints.push_back(w);
}

void aspc::Program::addRewrittenWeak(const aspc::Rule &rW)
{
    reWrittenWeaks.push_back(rW);
}

unsigned aspc::Program::getReWrittenWeakSize() const
{
    return reWrittenWeaks.size();
}

const aspc::Rule &aspc::Program::getRewrittenWeak(unsigned i) const
{
    return reWrittenWeaks[i];
}

const std::vector<aspc::Rule> &aspc::Program::getRewrittenWeak() const
{
    return reWrittenWeaks;
}

unsigned aspc::Program::getWeakSize() const
{
    return weakConstraints.size();
}

const std::vector<aspc::Rule> &aspc::Program::getWeak() const
{
    return weakConstraints;
}

const aspc::Rule &aspc::Program::getWeak(unsigned i) const
{
    return weakConstraints[i];
}

std::set<std::string> aspc::Program::getHeadRewrittenRule() const
{
    set<std::string> headRW;

    for (const aspc::Rule &rW : reWrittenWeaks)
    {
        for (const aspc::Atom &a : rW.getHead())
        {
            headRW.insert(a.getPredicateName());
        }
    }
    return headRW;
}
