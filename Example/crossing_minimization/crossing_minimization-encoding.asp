%%%%%%%%%%%%%%% We can consider only the decisional variant of the problem %%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Instances should be duplicated by varying the bound atom %%%%%%%%%%%%%%%%
pvalue(L,0) :- width(L,A).
pvalue(L,X1) :- pvalue(L,X), width(L,T), X < T, X1 = X+1.
position( Node, Pos ) :- in_layer( Layer, Node ), width( Layer, T ), Pos = P + 1, pvalue(Layer,P), P < T, not not_position( Node, Pos ).
not_position( Node, Pos ) :- in_layer( Layer, Node ), width( Layer, T ), Pos = P + 1, pvalue(Layer,P), P < T, not position( Node, Pos ).
