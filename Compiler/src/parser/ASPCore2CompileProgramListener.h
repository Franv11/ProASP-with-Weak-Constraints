...
private:
  ...
  // EDIT
  bool readingWeightConstraint;
  std::string weight;
  std::string level;
  std::set<std::string> termsInWhichPay;
...
public:
  virtual void enterProgram(ASPCore2Parser::ProgramContext * /*ctx*/) override
  {
    ...
    // EDIT
    readingWeightConstraint = false;
    level = "0";
  }

  bool checkTight()
  {
    std::cout << "Enter in checkTight function " << std::endl;
    bool tight = true;
    auto scc = pdg.SCC();
  ...
  virtual void exitSimple_rule(ASPCore2Parser::Simple_ruleContext * /*ctx*/) override
  {
    ...
    // check on BuildRels and buildingAggre for both rule and weak
    bool safe = true;
    for (int i = 0; i < buildingRels.size(); i++)
    {
      ...
      if (!safe)
      {
        std::cout << "body not safe " << std::endl;
        exit(180);
      }
    }
    if (safe)
    {
      for (std::unordered_set<std::string> vars : {negativeBodyVars, headVars})
      {
        ...
        if (!safe)
        {
          std::cout << "Safety Error :" << std::endl;
          exit(180);
        }
      }
    }
    if (readingWeightConstraint)
    {
      aspc::Rule weak(head, bodyLiterals, buildingRels, buildingAggregates, weight, level, termsInWhichPay);
      ...
      std::cout << "  ";
      std::cout << "[ " << weight << " @ " << level << " ,";
      for (auto t : termsInWhichPay)
      {
        std::cout << t << " ";
      }
      std::cout << " ]" << std::endl;
      ...
      // Rule for level
      // aspc::Rule ruleLevel(std::vector<aspc::Atom>{aspc::Atom("level", std::vector<std::string>{level})}, bodyLiterals, buildingRels, buildingAggregates);
      // ruleLevel.print();

      // RULE v_
      std::vector<std::string> attributes(termsInWhichPay.begin(), termsInWhichPay.end());
      attributes.insert(attributes.begin(), {weight, level});
      aspc::Rule v(std::vector<aspc::Atom>{aspc::Atom(std::string("v_") + std::to_string(attributes.size()), attributes)}, bodyLiterals, buildingRels, buildingAggregates);
      v.print();

      // cleaning and setting to default
      readingWeightConstraint = false;
      level = "0";
      termsInWhichPay.clear();

      // ADD TO PROGRAM
      program.addWeakConstraint(weak);
      program.addRewrittenWeak(v);
    }
    else
    {
    ...
    std::cout << "DataStructure after clear : " << std::endl;
    std::cout << "BuildingAtoms : " << buildingAtoms.size() << std::endl;
    std::cout << "buildingRels : " << buildingRels.size() << std::endl;
    std::cout << "buildingAggregates : " << buildingAggregates.size() << std::endl;
    std::cout << "buildingAtomsPolarity : " << buildingAtomsPolarity.size() << std::endl;
    std::cout << "bodyLiterals : " << bodyLiterals.size() << std::endl;
  }
      ...
      for (const aspc::Literal &literal : r.getBodyLiterals())
      {
        ...
        for (unsigned i = 0; i < headPredicateId.size(); i++)
        {
          if (!literal.isNegated())
            pdg.addEdge(predId, headPredicateId[i]);
  ...
  virtual void enterWeight_at_levels(ASPCore2Parser::Weight_at_levelsContext * /*ctx*/) override
  {
    ...
    // std::cout << "EnterWeight_AT_Levels, terminals.size() : " << atomStart << std::endl;
    readingWeightConstraint = true;
  }
  virtual void exitWeight_at_levels(ASPCore2Parser::Weight_at_levelsContext * /*ctx*/) override
  {
    // std::cout << "ExitWeight, terminals.size() : " << terminals.size() << std::endl;
    std::vector<std::string> terms;
    int expTermId = 0;

    for (int i = atomStart; i < terminals.size(); i++)
    {
    ...
    // Assert
    assert(terms.size() > 0);
    assert(std::find(terms.begin(), terms.end(), "@") == terms.end());
    ...
    if (terms.size() > 1)
    {
      for (int i = 1; i < terms.size(); i++)
      {
        ...
        {
          std::cout << "Term in termsInWhichPay is not a variable" << std::endl;
          exit(180);
        }
        termsInWhichPay.insert(terms[i]);
      }
    }

    // CHECK SAFETY
    std::set<std::string> wl{weight, level};

    for (std::set<std::string> vars : {wl, termsInWhichPay})
    {
      for (std::string var : vars)
      {
        if ((std::find(positiveBodyVars.begin(), positiveBodyVars.end(), var) == positiveBodyVars.end()) && isVariable(var))
        {
          std::cout << "Term not supported : " << var << std::endl;
          exit(180);
        }
      }
    }
    ...
    // CLEAR TERMINALS
    while (terminals.size() > atomStart)
    {
      terminals.pop_back();
    }
  }
  ...
  virtual void exitNaf_literal(ASPCore2Parser::Naf_literalContext * /*ctx*/) override
  {
    ...
    {
      if (naf)
        std::cout << "Found negated literal" << std::endl;
  ...
  {
    if (guardStartIndex > 0)
      isGuardExpression = true;
    expNestingLevel++;
  ...
  virtual void enterFactor(ASPCore2Parser::FactorContext * /*ctx*/) override
  {
    ...
    if (factorNestingLevel == 0)
      factorStartIndex = terminals.size();
  ...
  virtual void exitFactor(ASPCore2Parser::FactorContext * /*ctx*/) override
  {
    ...
    if (factorNestingLevel < 0)
    {
      ...
      if (expLength != 1 && expLength != 3)
      {
        std::cout << "Error parsing factor: found factor with length " << expLength << std::endl;
        exit(180);
      }
      if (expLength == 1)
      {
  ...
  {
    std::cout << "Error: leftward not supported yet. Coming soon ..." << std::endl;
    exit(180);
  }
  ...
  void endGuard()
  {
    ...
    while (terminals.size() > guardStartIndex)
      terminals.pop_back();
    std::cout << std::endl;
    guardStartIndex = -1;
  }
  ...
  virtual void enterAggregate_atom(ASPCore2Parser::Aggregate_atomContext * /*ctx*/) override
  {
    ...
    std::cout << "--------------- In enterAggregate_atom ------------------------------" << std::endl;
  ...
  virtual void exitAggregate_atom(ASPCore2Parser::Aggregate_atomContext * /*ctx*/) override
  {
    ...
    if (equalAggregate)
    {
      if (terminals.back() == "")
        ...
        guard = aspc::ArithmeticExpression(terminals.back());
      terminals.pop_back();
    ...
    while (terminals.size() > aggregateStartIndex + 1)
      terminals.pop_back();
    ...
    while (buildingAtoms.size() > aggrBodyLiteralStart)
    {
      buildingAtoms.pop_back();
      buildingAtomsPolarity.pop_back();
    }
    ...
    while (buildingRels.size() > aggrBodyIneqsStart)
      buildingRels.pop_back();
  ...
  {
    std::cout << "Parsing tree error node" << std::endl;
    exit(180);
  }
};
