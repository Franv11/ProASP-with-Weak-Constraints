...
#include "compilers/WeakInitCompiler.h"
...
int main(int argc, char *argv[])
{
	...
	for (unsigned ruleId = 0; ruleId < eagerProgram.getRulesSize(); ruleId++)
	{
		if (eagerLabels[ruleId])
		{
			if (!eagerProgram.getRule(ruleId).containsAggregate())
				traceToGroundLabeledRule[generatorRuleLabel.size()] = ruleId;
	...
	// set with weakRewritten head predicate
	std::set<std::string> rewrittenHead(reader.getInputProgram().getHeadRewrittenRule());

	WeakInitCompiler weakInitCompiler(rewrittenHead, executablePath, &dc);
	weakInitCompiler.compile();
