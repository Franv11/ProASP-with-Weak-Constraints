...
bool Analyzer::findAggregateNegativeDependency(const std::vector<std::vector<int>> &scc, unsigned componentId, const aspc::ArithmeticRelationWithAggregate *aggrRelation)
{
    for (const aspc::Literal &literal : aggrRelation->getAggregate().getAggregateLiterals())
    {
        ...
        bool found = false;
        for (unsigned id : scc[componentId])
            if (id == bodyPredicateId)
                found = true;
...
void Analyzer::labelArithmeticRelations(const std::vector<const aspc::Formula *> *formulas, std::vector<int> &formulaLabeling, std::unordered_set<std::string> &positiveVars)
{
    ...
    while (newRelationLabeled)
    {
        ...
        {
            if (formulas->at(i)->isLiteral() || formulas->at(i)->containsAggregate() || formulaLabeling[i] != UNK_FORMULA_LABEL)
                continue;
...
bool Analyzer::findMaximalDatalogBody(const aspc::Rule *rule, int ruleId, const std::vector<int> &sccLabel, std::unordered_map<std::string, int> &predToComponent, std::vector<int> &formulaLabeling)
{
    ...
    for (unsigned i = 0; i < formulas->size(); i++)
    {
        if (!formulas->at(i)->isLiteral() || !formulas->at(i)->isPositiveLiteral() || formulaLabeling[i] == UNK_FORMULA_LABEL)
            continue;
    ...
    for (unsigned i = 0; i < formulas->size(); i++)
    {
        if (!formulas->at(i)->isLiteral() || formulas->at(i)->isPositiveLiteral())
            continue;
    ...
    for (unsigned i = 0; i < formulas->size(); i++)
    {
        if (!formulas->at(i)->containsAggregate() || formulaLabeling[i] == NON_DATALOG_FORMULA)
            continue;
...
void Analyzer::rewriteWithJoin(const aspc::Rule *rule, int ruleId, std::vector<int> formulaLabeling, bool isConstraint)
{
    ...
    for (unsigned i = 0; i < rule->getFormulas().size(); i++)
    {
        ...
        {
            if (f->isLiteral())
                literalsIDB.push_back(*((const aspc::Literal *)f));
            else if (!f->containsAggregate())
                ineqsIDB.push_back(*((const aspc::ArithmeticRelation *)f));
            else
                aggregatesIDB.push_back(*((const aspc::ArithmeticRelationWithAggregate *)f));
        }
        else
        {
            if (f->isLiteral())
                literalsEDB.push_back(*((const aspc::Literal *)f));
            else if (!f->containsAggregate())
                ineqsEDB.push_back(*((const aspc::ArithmeticRelation *)f));
            else
                aggregatesEDB.push_back(*((const aspc::ArithmeticRelationWithAggregate *)f));
        }
    }
    ...
    for (unsigned i = 0; i < literalsIDB.size(); i++)
        reorderedLiterals.push_back(aspc::Literal(literalsIDB[i]));
    ...
    while (componentId >= 0)
    {
        for (int predicateId : scc[componentId])
        {
            ...
            for (int ruleId : rulesForPredicate)
            {
                ...
                {
                    if (sccLabel[componentId] == TYPE_LAZY)
                        lazyPrg.addRule(*rule);
                    else
                        datalogPrg.addRule(*rule);
                }
            }
...
void Analyzer::splitProgram()
{
    ...
    for (std::string hRw : program.getHeadRewrittenRule())
    {
        int idWeak = predicateToComponent[hRw];
        sccLazyLabel[idWeak] = NOT_LAZY;
    }
...
{
    if (joinRuleData.find(predicate) != joinRuleData.end())
        return true;
